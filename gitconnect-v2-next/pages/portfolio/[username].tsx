...
// const { username } = params;
  ...
  const initialProfile = Array.isArray(profileData) ? profileData[0]?.docData ?? null : profileData?.docData ?? null;

// const initialProjects: { id: string; name: string; description: string }[] = await fetch(
//   `https://api.github.com/users/${username}/repos?per_page=100`
// ).then((res) => res.json());

// const initialProfile = await fetch(`https://api.github.com/users/${username}`).then((res) => res.json());
...
export default function Portfolio({ initialProjects, initialProfile }: PortfolioProps) {
  ...
  const { data: projects } = useSWR(`/api/getUserProjects?username=${username}`, fetcher, initialProjects);
        ...
        <Grid.Col sm={12} md={3} lg={2}>
          {profile && (
            <ProfilePageUserPanel
              props={profile}
              currentUser={username && (userData.userName.toLowerCase() === username.toString().toLowerCase()) ? true : undefined}
