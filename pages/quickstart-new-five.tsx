...
const QuickstartPage = () => {
  ...
  // Handle error states if hooks return errors
  useEffect(() => {
  ...
  useEffect(() => {
    if (uiStep === 'selectRepos' && githubProfile && !anonymousUser) {
      createAnonymousSession(githubProfile).then((anonUser) => {
        if (!anonUser) {
          notifications.show({
            title: 'Authentication Error',
            message: 'Could not start an anonymous session.',
  ...
  const handlePortfolioBuildSubmit = async () => {
    if (!anonymousUser || !githubProfile || selectedRepoIds.length === 0) {
      notifications.show({
        ...
        message:
          'Cannot build portfolio. Ensure user is authenticated and repos are selected.',
        color: 'orange',
      });
      return;
    }
