...
const QuickstartPage = () => {
  ...
  // Fetch Github info and create anonymous user on submit
  const handleUsernameSubmit = async (currentUsername: string) => {
    notifications.show({
      id: "fetch-github",
      loading: true,
      title: "Fetching GitHub Data",
      message: `Looking for ${currentUsername}...`,
      autoClose: false,
    });
  ...
  // Effect to handle github fetch notifications
  useEffect(() => {
    if (!isGithubLoading && githubProfile && githubRepos) {
      ...
      notifications.update({
        ...
        loading: false,
        title: "Not Found",
        message: `GitHub user "${usernameInputValue}" not found or has no public repositories.`,
        color: "orange",
        autoClose: 5000,
      });
    }
  ...
  useEffect(() => {
    // if (builderProgress.stage === "enriching") {
    //   notifications.update({
    //     id: "portfolio-build",
    //     loading: true,
    //     title: "Fetching extra project data",
    //     message: `Updating ${builderProgress.current} of ${builderProgress.total} projects`,
    //     color: "cyan",
    //     autoClose: false,
    //   });
    // }
  ...
  // Handle notifications if hooks return errors
  useEffect(() => {
  ...
  return (
    <Container mt={80} size="lg" p="lg">
      ...
      {/* {overallLoading && uiStep === 'processing' && (
        <LoadingOverlay
          message={
            builderProgress.stage === 'enriching'
              ? 'Enriching projects...'
