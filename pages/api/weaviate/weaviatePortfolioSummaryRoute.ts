...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`Portfolio summary generation handler called with username: ${username}`)
  ...
  const generatedReponse = await generateUserWorkSummary(username.toString());
    ...
    res.status(500).json({ error: `Error generating portfolio summary from Weaviate with error: ${error}` });
  }
}
